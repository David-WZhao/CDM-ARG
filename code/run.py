...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        """
        Args:
            inputs: log probabilities (after log_softmax)
            targets: class indices
        """
        ce_loss = F.nll_loss(inputs, targets, weight=self.alpha, reduction='none')
...
# Print args without lr
args_dict = vars(args).copy()
args_dict.pop('lr', None)
print(f"Config: device={args.device}, batch_size={batch_size}, epoch={epoch}, K={K}, train_rate={train_rate}")
...
# Modified loss weights according to paper requirements
# La: antibiotic, Lm: mechanism, Lt: transfer, LDiff: diffusion
# Losstotal = α * La + β * Lm + γ * Lt + δ * LDiff
# alpha: antibiotic, beta: mechanism, yita(gamma): transfer, tao(delta): diffusion
alpha, beta, yita, tao = 1.0, 0.2, 0.2, 0.2
...
t_transfer_acc, t_transfer_precision, t_transfer_recall, t_transfer_f1, t_transfer_auc, t_transfer_aucpr = 0, 0, 0, 0, 0, 0
t_antibiotic_acc, t_antibiotic_precision, t_antibiotic_recall, t_antibiotic_f1, t_antibiotic_auc, t_antibiotic_aucpr = 0, 0, 0, 0, 0, 0
t_mechanism_acc, t_mechanism_precision, t_mechanism_recall, t_mechanism_f1, t_mechanism_auc, t_mechanism_aucpr = 0, 0, 0, 0, 0, 0
...
for k in range(K):
    ...
    # Focal Loss optimized for AUCPR improvement
    # Antibiotic (15 classes): moderate weights, gamma=1.8
    antibiotic_weights = torch.tensor([1.3, 0.85, 1.5, 1.1, 1.4, 0.9, 1.2, 1.5, 1.05, 0.9, 1.3, 1.2, 0.95, 1.1, 1.2]).to(device)
    # Mechanism (6 classes): INCREASED weights and gamma to boost AUCPR
    # Higher gamma (2.0) focuses more on hard examples, improving precision-recall curve
    mechanism_weights = torch.tensor([1.5, 0.85, 1.6, 1.2, 1.5, 0.9]).to(device)

    transfer_loss_function = nn.NLLLoss()
    antibiotic_loss_function = FocalLoss(alpha=antibiotic_weights, gamma=1.8)
    mechanism_loss_function = FocalLoss(alpha=mechanism_weights, gamma=2.0)
    ...
    for e in range(epoch):
        ...
        for index, (seq,seq_map, transfer_label, mechanism_label, antibiotic_label) in enumerate(train_dataloader):
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"WARNING: NaN/Inf loss detected at epoch {e+1}, batch {index+1}, skipping batch")
                optimizer.zero_grad()
                continue

            loss.backward()
            # Stronger gradient clipping for batch_size=32
        ...
        with torch.no_grad():
            for index, (seq, seq_map, transfer_label, mechanism_label, antibiotic_label) in enumerate(val_dataloader):
                ...
                # Process through ProteinBERT like in training
                ...
                antibiotic_output, mechanism_output, transfer_output, _ = model.forward(expanded_tensor,
        ...
        acc, macro_p, macro_r, macro_f1, auc, aucpr = evaluate(val_transfer_pred, val_transfer_label, transfer_count)
        print('transfer -> acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}'.format(acc, macro_p, macro_r, macro_f1, auc, aucpr))
        acc, macro_p, macro_r, macro_f1, auc, aucpr = evaluate(val_mechanism_pred, val_mechanism_label, mechanism_count)
        print('mechanism -> acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}'.format(acc, macro_p, macro_r, macro_f1, auc, aucpr))
        acc, macro_p, macro_r, macro_f1, auc, aucpr = evaluate(val_antibiotic_pred, val_antibiotic_label, antibiotic_count)
        print('antibiotic -> acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}'.format(acc, macro_p, macro_r, macro_f1, auc, aucpr))

        # Learning rate scheduling
        scheduler.step()

    # 测试 (moved outside epoch loop to run only once per fold)
    model.eval()
    ...
    t_antibiotic_auc += auc
    t_antibiotic_aucpr += aucpr

    torch.save(model.state_dict(), './res/model{}.pth'.format(k))

print('transfer => final acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}\n'.format(
    t_transfer_acc / K, t_transfer_precision / K, t_transfer_recall / K, t_transfer_f1 / K, t_transfer_auc / K, t_transfer_aucpr / K))

print('mechanism => final acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}\n'.format(
    t_mechanism_acc / K, t_mechanism_precision / K, t_mechanism_recall / K, t_mechanism_f1 / K, t_mechanism_auc / K, t_mechanism_aucpr / K))

print('antibiotic => final acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}\n'.format(
    t_antibiotic_acc / K, t_antibiotic_precision / K, t_antibiotic_recall / K, t_antibiotic_f1 / K, t_antibiotic_auc / K, t_antibiotic_aucpr / K))
# write the results to file (without hyperparameters)
with open('./res/result.txt', 'a', encoding='utf8') as f:
    f.write('\n transfer => final acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}\n'.format(
        t_transfer_acc / K, t_transfer_precision / K, t_transfer_recall / K, t_transfer_f1 / K, t_transfer_auc / K, t_transfer_aucpr / K))
    f.write('\n mechanism => final acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}\n'.format(
        t_mechanism_acc / K, t_mechanism_precision / K, t_mechanism_recall / K, t_mechanism_f1 / K, t_mechanism_auc / K, t_mechanism_aucpr / K))
    f.write('\n antibiotic => final acc: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}, auc: {:.4f}, aucpr: {:.4f}\n'.format(
        t_antibiotic_acc / K, t_antibiotic_precision / K, t_antibiotic_recall / K, t_antibiotic_f1 / K, t_antibiotic_auc / K, t_antibiotic_aucpr / K))
    f.write('----------------------------------------------------------------------------------------\n')
