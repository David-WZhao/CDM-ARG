...
class ARGDataSet(tud.Dataset):
    ...
    def __getitem__(self, item):
        return self.data.iloc[item]['seq'], torch.FloatTensor(self.data.iloc[item]['seq_map']), self.data.iloc[item]['type_label'], self.data.iloc[item]['mech_label'], self.data.iloc[item]['anti_label']
...
if __name__ == '__main__':
    ...
    index, (seq_map, anti_label, mech_label, type_label) = next(enumerate(dataloader))
    print(seq_map)
    # print(seq_map.size())
